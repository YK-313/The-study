...
if intrlv==1
    ...
    TX.pSym    = pskmod(double(TX.p),G.Q,pi/G.Q,InputType="bit");
...
RX.pA = fft(RX.pAA, fft_ptA)./sqrt(fft_ptA);%64個の受信シンボル
RX.pB = fft(RX.pAB, fft_ptB)./sqrt(fft_ptB);%64個の受信シンボル
RX.pN = fft(CH.pn, fft_ptB)./sqrt(fft_ptB);%64個の受信シンボル
...
switch(SIM.PilotMode)
    ...
    RX.p=RX.pB+RX.pA+RX.pN; %同時にパイロット送信
        ...
        switch(SIM.modeAB)
            ...
            P.EST.Xi=RX.p(1:SIM.ndata)./TX.pSym(:,1);%所望信号も雑音扱いでxiAA推定
            P.L = SIM.delayA * num_of_paths_AA; % 有効な最大遅延サンプル数 (これ以降の時間は0)
            P.F_matrix = fft(eye(fft_ptA)); % 128ポイントのFFT行列を生成
            P.F_partial = P.F_matrix(1:SIM.ndata, 1:P.L);
            P.F_H = P.F_partial';% 共役転置
            %LS
            P.EST.h_L = pinv(P.F_partial) * P.EST.Xi; % 擬似逆行列(pinv)を用いて、64個の周波数成分からL個の時間タップを逆算
            P.EST.h = [P.EST.h_L; zeros(fft_ptA - P.L, 1)]; % 128ポイントのインパルス応答として再構成 (L以降はゼロ詰め＝完全な窓関数)
            P.EST.Xi_vec_AA = fft(P.EST.h, fft_ptA);% 128ポイントFFTで周波数領域へ
            P.EST.XiHat = P.EST.Xi_vec_AA(1:SIM.ndata);% 3. 最終的なチャネル推定値 (BCJRなどのデータ復号用)
            TildePilot=RX.p(1:SIM.ndata)-P.EST.XiHat.*TX.pSym(:,1); 
            EST.XivecAB =TildePilot./TX.pSym(:,2);
    
            case {'DASIC1'}
            phiPilot = TX.pSym(2:end,1) ./ TX.pSym(1:end-1,1);  % 位相シフトを計算
            RX.pD=zeros(SIM.ndata,1);
            RX.pD(1)=RX.p(1);
            RX.pD(2:SIM.ndata) = RX.p(2:SIM.ndata) -  phiPilot.*RX.p(1:SIM.ndata-1); %自己干渉除去
            EST.XivecAB = RX.pD(1:SIM.ndata)./TX.pSym(:,2);
            ...
            RX.pD(1)=RX.p(1);
            RX.pD(2:SIM.ndata) = RX.p(2:SIM.ndata) -  phiPilot.*RX.p(1:SIM.ndata-1); %自己干渉除去
            RX.pD2=zeros(SIM.ndata,1);
            RX.pD2(1)=RX.p(1);
            RX.pD2(2)=RX.p(2);
            RX.pD2(3:SIM.ndata) = RX.pD(3:SIM.ndata) -  phiPilot(2:end).*RX.pD(2:SIM.ndata-1); %自己干渉除去
            EST.XivecAB = RX.pD2(1:SIM.ndata)./TX.pSym(:,2);
        end
end

    Lp = SIM.delayB * num_of_paths_AB; % 有効な最大遅延サンプル数 (これ以降の時間は0)
    F_matrix_p = fft(eye(fft_ptB)); % 128ポイントのFFT行列を生成
    F_partial_p = F_matrix_p(1:SIM.ndata, 1:Lp);
    EST.h_LP = pinv(F_partial_p) * EST.XivecAB; % 擬似逆行列(pinv)を用いて、64個の周波数成分からL個の時間タップを逆算
    EST.hP = [EST.h_LP; zeros(fft_ptB - Lp, 1)]; % 128ポイントのインパルス応答として再構成 (L以降はゼロ詰め＝完全な窓関数)
    EST.Xi_vec_AB = fft(EST.hP, fft_ptB);% 128ポイントFFTで周波数領域へ
    EST.HatXivecAB = EST.Xi_vec_AB(1:SIM.ndata);% 3. 最終的なチャネル推定値 (BCJRなどのデータ復号用)
...
EST.Xi=(RX.b(1:SIM.ndata)-EST.HatXivecAB(1:SIM.ndata).*xbhat)./TX.x(:,1);%推定した所望信号を減算して
end
...
switch SIM.detmode
    case'MLD'
tilde_xb=(RX.b-EST.XiHat.*TX.x(:,1))./EST.HatXivecAB;%SIのシンボル減算し，xiABで割る
