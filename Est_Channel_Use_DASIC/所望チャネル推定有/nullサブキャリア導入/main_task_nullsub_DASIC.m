...
function SIM = main_task_nullsub_DASIC(En,idx,SIM,G)
...
switch(SIM.null)
    ...
    boundary_idx = 1;
    for i = 1:num_data_subc-1
        if freq_ordered_idx(i+1) < freq_ordered_idx(i)
            boundary_idx = i;
            break;
        end
    end
    
    pad_zeros = zeros(bcjr_pad_bits / 2, 1); % 4ビット
    
    % 前半(Chunk 1)に入るはずのデータビット数を計算
    % これにより、並び替え(ordered)後にpad_zerosがピタリと配列の「両端」に来ます
    idx_data1 = boundary_idx * bits_per_symbol - length(pad_zeros);
    
    TX.codedata_int = zeros(total_bits, 2);
    TX.codedata_int(:,1) = [TX.codedata_in(1:idx_data1, 1); pad_zeros; pad_zeros; TX.codedata_in(idx_data1+1:end, 1)];
    TX.codedata_int(:,2) = [TX.codedata_in(1:idx_data1, 2); pad_zeros; pad_zeros; TX.codedata_in(idx_data1+1:end, 2)];
...
switch(SIM.PilotMode)
    case 'alter'
        ...
        % 【修正】全帯域ではなく、有効サブキャリア(data_idx)のみで計算する
        EST.XivecAB_valid = RX.p(data_idx)./TX.pSym(data_idx,2); 
        
    case 'simul'
        RX.p=RX.pB+RX.pA+RX.pN; %同時にパイロット送信
        switch(SIM.modeAB)
            case {'WoSIC'}
                % 【修正】こちらも有効サブキャリアのみで計算
                EST.XivecAB_valid = RX.p(data_idx)./TX.pSym(data_idx,2); 
            ...
            case {'SIC'}
                ...
                P.L = SIM.delayA * num_of_paths_AA; 
                P.F_matrix = fft(eye(fft_ptA)); 
                phys_valid_idx_A = phys_idx_A(data_idx);
                
                P.F_partial_valid = P.F_matrix(phys_valid_idx_A, 1:P.L);
                P.EST.h_L = pinv(P.F_partial_valid) * P.EST.Xi_valid; 
                P.EST.h = [P.EST.h_L; zeros(fft_ptA - P.L, 1)]; 
                P.EST.Xi_vec_AA = fft(P.EST.h, fft_ptA);
                P.EST.XiHat = P.EST.Xi_vec_AA(phys_idx_A);
...
% (ここからは先ほどのコードのままでOKです)
Lp = SIM.delayB * num_of_paths_AB; 
F_matrix_p = fft(eye(fft_ptB)); 

phys_valid_idx_B = phys_idx_B(data_idx);

F_partial_p_valid = F_matrix_p(phys_valid_idx_B, 1:Lp);
EST.h_LP = pinv(F_partial_p_valid) * EST.XivecAB_valid; 
EST.hP = [EST.h_LP; zeros(fft_ptB - Lp, 1)]; 
EST.Xi_vec_AB = fft(EST.hP, fft_ptB); 

% 128ptのFFT結果からベースバンド(64個)を抽出
EST.HatXivecAB = EST.Xi_vec_AB(phys_idx_B);

    %nullサブキャリアの除去・有効なサブキャリアのみを抽出
    RX.b_valid = RX.b(data_idx);
    TX.x_valid = TX.x(data_idx, 1);
    %DASIC用に周波数軸順に並び替え
    RX.b_ordered = RX.b(freq_ordered_idx);
    TX.x_ordered = TX.x(freq_ordered_idx, 1);
    EST.HatXivecAB_ordered = EST.HatXivecAB(freq_ordered_idx);

    switch(SIM.mode)
    case {'xb_est1','xb_est2','DASIC1','DASIC2'}
    
        phi = TX.x_ordered (2:end) ./ TX.x_ordered (1:end-1); % 連続した周波数での位相シフト
        
        RX.c_ordered = zeros(num_data_subc, 1);
        RX.c_ordered(1) = RX.b_ordered(1);
        RX.c_ordered(2:num_data_subc) = RX.b_ordered(2:num_data_subc) - phi .* RX.b_ordered(1:num_data_subc-1);
        
        RX.c2_ordered = zeros(num_data_subc, 1);
        RX.c2_ordered(1) = RX.b_ordered(1);
        RX.c2_ordered(2) = RX.b_ordered(2);
        RX.c2_ordered(3:num_data_subc) = RX.c_ordered(3:num_data_subc) - phi(2:end) .* RX.c_ordered(2:num_data_subc-1);
    end
    ...
    case {'xb_est'}
            ...
            TX.Xi_vec_AB = EST.HatXivecAB_ordered;
            ...
            for xx = 2: num_data_subc-1
                ...
                for idx_in = 1:trel.num_in
                    ...
                    BCJR.Gamma(sigi,trel.next_state(sigi,idx_in)+1,xx) = (-1*(abs(RX.b_ordered(xx+1)-trel.outputs(sigi,idx_in)))^2)/2/CH.N0;%尤度の計算
                    end
                end
            ...
            %% BCJRからの判定
            %パディング(各4bit)を除外した純粋なデータL値を抽出
            L_ordered_data = BCJR.L(3:end-4);
            
            % orderedで反転した順序(後半->前半)を元の順序(前半->後半)に戻す
            chunk2_len = length(L_ordered_data) - idx_data1;
            BCJR.L_valid = [L_ordered_data(chunk2_len+1 : end); L_ordered_data(1 : chunk2_len)];
            
            if intrlv==1
                BCJR.a = randdeintrlv(BCJR.L_valid, 1);
            else
                BCJR.a = BCJR.L_valid;
            end
            ...
            % 推定所望信号シンボルの再構成（※最終共通BCJRには不要です）
            A = det.decode(1:end-conv_tail_bits);
            AA = step(ConEnc,A);
            AAA = randintrlv(round(AA),1);
            
            % 再構成時も送信側と全く同じ位置(idx_data1)にパディングを挿入
            AAAA = [AAA(1:idx_data1); pad_zeros; pad_zeros; AAA(idx_data1+1:end)];
    ...
    case {'xb_est1','DASIC1'}
        ...
        for xx = 2:length(RX.c_ordered)-1
                trel1 = BCJRTrellisDasic(TX,xx,CH,G,0,1);
            for idx_in = 1:trel1.num_in
                for sigi=1:trel1.num_state %状態i
                    BCJR1.Gamma(sigi,trel1.next_state(sigi,idx_in)+1,xx) = (-1*(abs(RX.c_ordered(xx+1)-trel1.outputs(sigi,idx_in)))^2)/2/CH.N0;%尤度の計算
                end
            end
        ...
        for xx = length(RX.c_ordered):-1:2
        ...
        %% BCJR1からの判定
        L_ordered_data = BCJR1.L(3:end-4);
            
            % orderedで反転した順序(後半->前半)を元の順序(前半->後半)に戻す
            chunk2_len = length(L_ordered_data) - idx_data1;
            BCJR1.L_valid = [L_ordered_data(chunk2_len+1 : end); L_ordered_data(1 : chunk2_len)];
            
            if intrlv==1
                BCJR1.a = randdeintrlv(BCJR1.L_valid, 1);
            else
                BCJR1.a = BCJR1.L_valid;
            end
    ...
    case {'xb_est2','DASIC2'}
        ...
        TX.Xi_vec_AB = EST.HatXivecAB_ordered;
        TX.phi=phi;
        for xx = 2:length(RX.c2_ordered)-1
            trel2 = BCJRTrellis2Dasic(TX,xx,CH,G,0,1);
            for idx_in = 1:trel2.num_in
                for state=1:16 %状態数(二段だと16)
                    BCJR2.Gamma(state,trel2.next_state(state,idx_in)+1,xx) = (-1*(abs(RX.c2_ordered(xx+1)-trel2.outputs(state,idx_in)))^2)/6/CH.N0;
                end
            end
            for sigk = 1:16 % 次状態
        ...
        for xx = length(RX.c2_ordered):-1:2
        ...
        %% BCJR2からの判定
            L_ordered_data = BCJR2.L(3:end-4);
            
            % orderedで反転した順序(後半->前半)を元の順序(前半->後半)に戻す
            chunk2_len = length(L_ordered_data) - idx_data1;
            BCJR2.L_valid = [L_ordered_data(chunk2_len+1 : end); L_ordered_data(1 : chunk2_len)];
            
            if intrlv==1
                BCJR2.a = randdeintrlv(BCJR2.L_valid, 1);
            else
                BCJR2.a = BCJR2.L_valid;
            end
    ...
    case 'est'
        ...
        EST.Xi_valid = RX.b_valid ./ TX.x(data_idx, 1);
    
    case {'xb_est','xb_est1','xb_est2'}
        % 推定した所望信号を減算してXi_AA推定 (有効サブキャリアのみ)
        EST.Xi_valid = (RX.b_valid - EST.HatXivecAB(data_idx) .* xbhat) ./ TX.x(data_idx, 1);
    end
    switch SIM.mode
    case {'est','xb_est','xb_est1','xb_est2'}
        ...
        % 行: 有効サブキャリア(data_idx), 列: パス数(1~L) の部分行列を抽出
        ...
        % 3. 最終的なチャネル推定値 (全64サブキャリア分として綺麗に補間・復元される)
    ...
    case {'Perfect','est','xb_est','xb_est1','xb_est2'}
        EST.XiHat_ordered = EST.XiHat(freq_ordered_idx);
        tilde_xb_ordered = RX.b_ordered - EST.XiHat_ordered .* TX.x_ordered;
        %% BCJR
        ...
        for xx = 2:num_data_subc-1
            ...
            for idx_in = 1:trel.num_in
                for sigi=1:trel.num_state %状態i
                    BCJR.Gamma(sigi,trel.next_state(sigi,idx_in)+1,xx) = (-1*(abs(tilde_xb_ordered(xx+1)-trel.outputs(sigi,idx_in)))^2)/2/CH.N0;%尤度の計算
                end
            end
