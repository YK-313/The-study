    ...
    RX.p=RX.pB+RX.pA+RX.pN; %同時にパイロット送信
        ...
        switch(SIM.modeAB)
          ...
          %% SIのレプリカを作成し，減算後パイロットからXi_ABを推定
            ...
            % 行: 有効サブキャリア(data_idx), 列: パス数(1~P.L) の部分行列を抽出
            ...
            P.EST.XiHat = P.EST.Xi_vec_AA(1:SIM.ndata); % 最終的なチャネル推定値 (全帯域復元)
            ...
            % 行: 有効サブキャリア(data_idx), 列: パス数(1~Lp) の部分行列を抽出
            ...
            EST.HatXivecAB = EST.Xi_vec_AB(1:SIM.ndata); % 最終的なチャネル推定値 (全帯域復元)
