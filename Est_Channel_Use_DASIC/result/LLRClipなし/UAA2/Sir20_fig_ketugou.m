...
fig1 = openfig( "WoXbEstSir20.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
...
fig2 = openfig( "XbEstSir20.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig3 = openfig( "1EstSir20.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig4 = openfig( "2EstSir20.fig", 'invisible'); % .figファイルを開く
ax = gca; 
...
hold on
l(2) = semilogy((xData2), yData2, 'bo-');
l(3) = semilogy((xData3), yData3, "Marker", "o", "Color", "#77AC30");
l(4) = semilogy((xData4), yData4, 'yo-');
% l(1) = semilogy((xData1), yData1, 'ro-');
% hold on
...
grid on;
legend ('所望信号推定なし' ,'所望信号を推定 (DASICなし)' ,'所望信号を推定 (一段DASIC)' ,'所望信号を推定 (ニ段DASIC)','Perfect CSI','FontSize', 20,'FontName', 'Times New Roman','Interpreter','latex' );
