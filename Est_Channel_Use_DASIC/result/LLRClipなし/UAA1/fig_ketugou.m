...
fig1 = openfig( "WoXbEstSir0.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
...
fig2 = openfig( "WoXbEstSir-20.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig3 = openfig( "WoXbEstSir-40.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig4 = openfig( "WoXbEstSir-60.fig", 'invisible'); % .figファイルを開く
ax = gca; 
...
fig5 = openfig( "WoXbEstSir-80.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig6 = openfig( "WoXbEstSir-100.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig14 = openfig( "Perfect.fig", 'invisible'); % .figファイルを開く
ax = gca; 
lines = findobj(ax, 'Type', 'line'); 
for i = 1:length(lines)
    xData14 = get(lines(i), 'XData'); % x軸のデータを取得
    yData14 = get(lines(i), 'YData'); % y軸のデータを取得
end
close(fig14);
...
l(14) = semilogy(xData14, yData14, 'k--');
...
grid on;
legend ('$\eta=0$ dB' ,'$\eta=-20$ dB' ,'$\eta=-40$ dB' ,'$\eta=-60$ dB','$\eta=-80$ dB' ,'$\eta=-100$ dB','Perfect CSI','FontSize', 20,'FontName', 'Times New Roman','Interpreter','latex' );
...
set(l(14), 'linewidth', 2, 'MarkerSize', 20, 'MarkerFaceColor', 'none');
