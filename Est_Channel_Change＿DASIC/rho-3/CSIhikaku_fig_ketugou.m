mode = 'Eb/N0'; %横軸の設定(SNR,Eb/N0)


fig1 = openfig( "AutoSir-50.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
...
fig2 = openfig( "AutoSir-40.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig3 = openfig( "AutoSir-30.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig4 = openfig( "AutoSir-20.fig", 'invisible'); % .figファイルを開く
ax = gca; 
...
%{
fig5 = openfig( "2DasicSir-50.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig6 = openfig( "2DasicSir-40.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig9 = openfig( "CheatXiAB_AutoSir-50.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig10 = openfig( "CheatXiAB_AutoSir-40.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig11 = openfig( "CheatXiAB_AutoSir-30.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig12 = openfig( "CheatXiAB_AutoSir-20.fig", 'invisible'); % .figファイルを開く
ax = gca; 
lines = findobj(ax, 'Type', 'line'); 
for i = 1:length(lines)
...
figure('Position', [1, 71, 813, 725]);
l(1) = semilogy((xData1)-3, yData1,"Marker", "o", "Color", "#7E2F8E");
hold on
l(2) = semilogy((xData2)-3, yData2, "Marker", "square", "Color", "#7E2F8E");
l(3) = semilogy((xData3)-3, yData3, "Marker", "^", "Color", "#7E2F8E");
l(4) = semilogy((xData4)-3, yData4, "Marker", "v", "Color", "#7E2F8E");
...
grid on;
legend ('$\eta=-50$ dB (w/o perfect CSI)' ,'$\eta=-40$ dB (w/o perfect CSI)' ,'$\eta=-30$ dB (w/o perfect CSI)' ,'$\eta=-20$ dB (w/o perfect CSI)','$\eta=-50$ dB (w/ perfect CSI)' ,'$\eta=-40$ dB (w/ perfect CSI)' ,'$\eta=-30$ dB (w/ perfect CSI)' ,'$\eta=-20$ dB (w/ perfect CSI)','HD','FontSize', 20,'FontName', 'Times New Roman','Interpreter','latex' );
