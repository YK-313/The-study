...
fig1 = openfig( "1DasicSir-50.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
...
fig2 = openfig( "1DasicSir-40.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig3 = openfig( "1DasicSir-30.fig", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig4 = openfig( "1DasicSir-20.fig", 'invisible'); % .figファイルを開く
ax = gca; 
...
grid on;
legend ('$\eta=-50$ dB (One)' ,'$\eta=-40$ dB (One)' ,'$\eta=-30$ dB (One)' ,'$\eta=-20$ dB (One)','$\eta=-50$ dB (Two)' ,'$\eta=-40$ dB (Two)' ,'$\eta=-30$ dB (Two)' ,'$\eta=-20$ dB (Two)','$\eta=-50$ dB (Adaptive)' ,'$\eta=-40$ dB (Adaptive)' ,'$\eta=-30$ dB (Adaptive)' ,'$\eta=-20$ dB (Adaptive)','HD','FontSize', 20,'FontName', 'Times New Roman','Interpreter','latex' );
