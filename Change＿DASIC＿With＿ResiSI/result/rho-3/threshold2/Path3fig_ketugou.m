...
fig1 = openfig( "AutoDasicSir-50Path3(rho-3)", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
...
fig2 = openfig( "AutoDasicSir-40Path3(rho-3)", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig3 = openfig( "AutoDasicSir-30Path3(rho-3)", 'invisible'); % .figファイルを開く
ax = gca; % 現在の軸を取得
lines = findobj(ax, 'Type', 'line'); % ラインオブジェクトを取得
for i = 1:length(lines)
...
fig4 = openfig( "AutoDasicSir-20Path3(rho-3)", 'invisible'); % .figファイルを開く
ax = gca; 
...
figure('Position', [1, 71, 813, 725]);
l(1) = semilogy((xData1)-3, yData1,"Marker", "o", "Color", "#A55A0A");
hold on
l(2) = semilogy((xData2)-3, yData2, "Marker", "square", "Color", "#A55A0A");
l(3) = semilogy((xData3)-3, yData3, "Marker", "^", "Color", "#A55A0A");
l(4) = semilogy((xData4)-3, yData4, "Marker", "v", "Color", "#A55A0A");
...
grid on;
legend ('$\eta=-50$ dB $(U_{\mathrm{AA}}=3)$', '$\eta=-40$ dB $(U_{\mathrm{AA}}=3)$', '$\eta=-30$ dB $(U_{\mathrm{AA}}=3)$', '$\eta=-20$ dB $(U_{\mathrm{AA}}=3)$', '$\eta=-50$ dB $(U_{\mathrm{AA}}=2)$', '$\eta=-40$ dB $(U_{\mathrm{AA}}=2)$','$\eta=-30$ dB $(U_{\mathrm{AA}}=2)$','$\eta=-20$ dB $(U_{\mathrm{AA}}=2)$','HD','FontSize', 20,'FontName', 'Times New Roman','Interpreter','latex' );
