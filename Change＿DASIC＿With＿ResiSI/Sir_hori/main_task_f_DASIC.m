...
function SIM = main_task_f_DASIC(SIR,En,idx,SIM,G)
...
if intrlv==1
    ...
    if SIM.AA==1
        ...
        delay_profile_AA_sir=delay_profile_AA_rho*sqrt(P_b*10^(-SIR/10));
...
%SIR = 10*log10(sum(ERR.tx_pow)/(sum(ERR.rx_pow)-sum(ERR.tx_pow)));
